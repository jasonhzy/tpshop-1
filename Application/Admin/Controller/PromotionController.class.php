...
/********************         一元购         *******************************/
    public function oneyuan_sale(){
        $condition = array();
        $model = M('oneyuan_sale');
        $count = $model->where($condition)->count();
        $Page  = new \Think\Page($count,10);         
        $show = $Page->show();
        $prom_list = $model->where($condition)->order("id desc")->limit($Page->firstRow.','.$Page->listRows)->select();
    ...
    public function oneyuan_sale_info(){
        if(IS_POST){
            $data = I('post.');
            $data['start_time'] = strtotime($data['start_time']);
            $data['end_time'] = strtotime($data['end_time']);
            if(empty($data['id'])){
                $r = M('oneyuan_sale')->add($data);
                //$string= M()->getlastsql();
                M('goods')->where("goods_id=".$data['goods_id'])->save(array('prom_id'=>$r,'prom_type'=>11));
                //$string= M()->getlastsql();
                adminLog("管理员添加抢购活动 ".$data['name']);
            }else{
                $r = M('oneyuan_sale')->where("id=".$data['id'])->save($data);
                M('goods')->where("prom_type=11 and prom_id=".$data['id'])->save(array('prom_id'=>0,'prom_type'=>0));
                M('goods')->where("goods_id=".$data['goods_id'])->save(array('prom_id'=>$data['id'],'prom_type'=>11));
            }
            if($r){
                $this->success('编辑抢购活动成功',U('Promotion/oneyuan_sale'));
                exit;
            }else{
                $this->error('编辑抢购活动失败',U('Promotion/oneyuan_sale'));
            }
        }
        $id = I('id');
        $info['start_time'] = date('Y-m-d H:i:s');
        $info['end_time'] = date('Y-m-d 23:59:59',time()+3600*24*60);
        if($id>0){
            $info = M('oneyuan_sale')->where("id=$id")->find();
            $info['start_time'] = date('Y-m-d H:i',$info['start_time']);
            $info['end_time'] = date('Y-m-d H:i',$info['end_time']);
        }
        $this->assign('info',$info);
    ...
    public function oneyuan_sale_del(){
        $id = I('del_id');
        if($id){
            M('oneyuan_sale')->where("id=$id")->delete();
            M('goods')->where("prom_type=11 and prom_id=$id")->save(array('prom_id'=>0,'prom_type'=>0));
            exit(json_encode(1));
        }else{
            exit(json_encode(0));
        }
